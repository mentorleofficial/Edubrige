  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BookingDetailRow> => {
      const { data, error } = await supabase
        .from("sessions")
        .select(
          "id, scheduled_at, duration_minutes, status, title, topic, notes, mentee_notes, meeting_url, cancellation_reason, cancelled_at, mentee_id, mentor_id, offering_id, program_id, mentee:users!sessions_mentee_id_fkey(full_name, avatar_url, email), offering:mentorship_offerings(id, title, price, currency, category), program:programs(id, name, color, slug), feedback(rating, comment, audience)"
        )
        .eq("id", bookingId!)
        .maybeSingle();
