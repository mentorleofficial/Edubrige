...
// State & API
import { useMentorEvents, useMentorEventsMutations } from "@/features/mentor-events/useMentorEvents";
...
export default function MentorEvents() {
  ...
  // Queries
  const { data: events = [], isLoading } = useMentorEvents(user?.id);
  const { createEvent, updateEvent, deleteEvent } = useMentorEventsMutations(user?.id);
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1.5">
              Create and manage your workshops, bootcamps, and guest speaking sessions
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-card border rounded-lg">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Loading your events...</p>
          </div>
          ...
          <div className="text-center py-16 bg-card border rounded-lg">
            ...
            <p className="text-xs text-muted-foreground max-w-sm mx-auto mb-6">
              {events.length === 0 
                ? "Host a webinar, workshop, or bootcamp to engage with your mentees and build your brand."
                : activeTab === "upcoming"
                  ? "Create a new event now to get started."
                  : activeTab === "past"
                    ? "Your past completed sessions will appear here."
